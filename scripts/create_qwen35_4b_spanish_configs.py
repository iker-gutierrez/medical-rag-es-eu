#!/usr/bin/env python
"""Generate 132 Qwen/Qwen3.5-4B Spanish dev configs (11 conditions × 3 dev sets × 2 think modes × 2 SF modes).

noSF IDs 448-513, SF IDs 514-579.
  448-469: SNS1064   (11 no_think + 11 think)
  470-491: CasiMedicos (11 no_think + 11 think)
  492-513: mixed       (11 no_think + 11 think)
  514-535: SNS1064 SF
  536-557: CasiMedicos SF
  558-579: mixed SF
"""
...
def make_sf(nosf_id, dev_slug, dev_input, exp_slug, partial, think):
    ...
    mode = "think" if think else "no_think"
    name = f"qwen35_4b_{exp_slug}_{mode}_extractive_{dev_slug}_sf"
    nosf_name = f"qwen35_4b_{exp_slug}_{mode}_extractive_{dev_slug}"
    cfg = {
        "experiment_name": name,
        "input": dev_input,
