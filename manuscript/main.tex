...
\usepackage{tikz}
\usepackage{float}
\usepackage[colorlinks=true,linkcolor=darkblue,citecolor=darkblue,urlcolor=darkblue,linktoc=page]{hyperref}
\definecolor{darkblue}{rgb}{0,0,0.5}
% Use starred \pageref* so the page number header has no hyperlink
\AtBeginDocument{\fancyhead[R]{\thepage/\pageref*{LastPage}}}
...
Retrieval-augmented generation (RAG) augments language models with external knowledge retrieved at inference time, following a ``retrieve-then-read'' paradigm \citep{liangEtAl2025}.
Early systems relied on sparse retrievers such as BM25, which were later superseded by dense retrievers like DPR and domain-specific models such as MedCPT \citep{liangEtAl2025}.
Advanced RAG systems extend this with multi-round structures including iterative, recursive, and adaptive retrieval.
More recently, agentic RAG frameworks treat retrieval and chain-of-thought reasoning as actions invocable by the model based on its own metacognitive assessment of when external knowledge is needed \citep{liangEtAl2025}.

A key finding across multiple studies is that RAG does not unconditionally improve performance: irrelevant or noisy retrieved passages can distract models and sustain hallucinations, an effect that is especially pronounced in smaller models \citep{kimEtAl2025}.
\citet{kimEtAl2025} conducted the first large-scale expert evaluation of medical RAG, with over 80,000 annotations across 200 queries, and found that simple mitigation strategies such as evidence filtering and query reformulation substantially recover the lost performance.
Complementing this, \citet{ozakiEtAl2025} show that models can exhibit calibrated confidence about whether retrieved documents are relevant, suggesting that models possess some capacity for self-assessment of retrieval quality.

\subsection{RAG for medicine}

Medical RAG has been benchmarked systematically by \citet{xiongEtAl2024}, who introduced MIRAGE — a benchmark of 7,663 questions from five medical QA datasets — and showed that MedRAG improves LLM accuracy by up to 18\% over chain-of-thought prompting alone.
Their study identified a log-linear scaling effect and a ``lost-in-the-middle'' phenomenon in medical RAG, whereby evidence placed in the middle of a long context is less effectively utilised.
\citet{sivakumarEtAl2026} extend this line of work with RAG-X, a diagnostic framework that evaluates the retriever and generator independently across information extraction, short-answer generation, and multiple-choice QA, introducing Context Utilisation Efficiency (CUE) metrics to isolate verified grounding from coincidental accuracy.

Patient-specific knowledge poses an additional challenge: \citet{liangEtAl2025} propose RGAR, which retrieves both factual knowledge from electronic health records and conceptual knowledge from a general corpus simultaneously, consistently improving generation quality and clinical relevance compared to single-source retrieval.

\subsection{Self-feedback and self-reflection}

Self-RAG \citep{asaiEtAl2023} introduced the idea of training a model to adaptively retrieve passages on demand and critique its own generation using special reflection tokens, achieving strong performance across a diverse set of tasks without requiring task-specific fine-tuning.
The self-feedback mechanism in our system is a simpler, inference-only variant: the model generates an initial answer and then receives a second prompt asking it to revise it, without any training signal.
A related application appears in OpenScholar \citep{asaiEtAl2026}, where a self-feedback-guided generation mechanism enables iterative refinement of long-form scientific synthesis, outperforming GPT-4o by 6.1\% on a multi-paper synthesis benchmark while producing far fewer hallucinated citations.
...
The dataset schema is presented in \autoref{tab:sns-schema}.
...
\autoref{tab:sns-totals} summarises the size of the dataset in terms of guidebooks, samples, sentences, and word tokens:
...
\autoref{tab:sns-guidebook-dist} shows the distribution of samples across the six source guidebooks, illustrating the coverage of different clinical domains.
...
In addition to global counts, per-feature statistics are reported in \autoref{tab:sns-length-stats}.
...
An overview of the dataset construction pipeline is illustrated in \autoref{fig:sns-pipeline}:
...
The segmentation logic is composed of the following patterns to identify input features (\autoref{tab:sns-input-triggers}) and output features (\autoref{tab:sns-output-triggers}):
...
\autoref{tab:sns-intermediate-optional} shows the presence rates of optional features.
...
\autoref{tab:sns-intermediate-completeness} shows that the extraction pipeline achieves near-complete presence rates for mandatory features, with noise affecting the \textit{judgement} and \textit{evidence} features.
...
\autoref{tab:sns-split} shows the resulting data distribution:
...
Results show a fully consistent dataset with respect to mandatory features (\autoref{tab:sns-final-completeness}).
...
\autoref{tab:sns-final-optional} compares optional feature distributions before and after manual curation.
...
\begin{itemize}
    \item \textbf{SNS-1064} (described in \autoref{sec:sns1064}): open-answer clinical QA derived from Spanish clinical guidebooks.
...
\autoref{tab:es-main-sns} covers the SNS1064 dev set (open-answer, 106 examples).
...
\input{main_table_es_sns1064}

\autoref{tab:es-main-casi} covers the CasiMédicos-Arg dev set (multiple-choice, 55 examples).
Because answers are short option labels, BERT-F1 and Cosim are more diagnostic than ROUGE-L here.

\input{main_table_es_casimedicos}

\autoref{tab:es-main-mixed} covers the mixed dev set (SNS1064 + CasiMédicos-Arg combined, 161 examples),
providing a single aggregate view across both task types.

\input{main_table_es_mixed}
...
\autoref{tab:eu-main-sns} covers the Basque SNS1064 EU dev set (open-answer, 106 examples).

\input{main_table_eu_sns1064_eu}

\autoref{tab:eu-main-casi} covers the Basque CasiMédicos-Arg EU dev set (multiple-choice, 55 examples).

\input{main_table_eu_casimedicos_eu}

\autoref{tab:eu-main-mixed} covers the Basque mixed dev set (161 examples).

\input{main_table_eu_mixed_eu}
...
To assess whether the larger model is worth the extra compute, \autoref{tab:4b-vs-9b} compares the best configuration found for each model size on each Spanish dataset.
...
\toprule
\textbf{Dataset} & \textbf{Model} & \textbf{Best config} & \textbf{ROUGE-L} & \textbf{Cosim} & \textbf{BERT-F1} & \textbf{sec} & \textbf{tok} \\
\midrule
SNS-1064        & 4B & 3-shot + rerank5, noSF & 38.69 & 89.35 & 78.81 &  3.49 & 1983 \\
                & 9B & 3-shot + rerank5, noSF & 38.68 & 89.61 & 78.91 &  4.10 & 1989 \\
                & \multicolumn{2}{r}{$\Delta$ (9B $-$ 4B)} & $-0.01$ & $+0.26$ & $+0.10$ & $+0.61$ & $+6$ \\
\midrule
CasiMédicos-Arg & 4B & 3-shot + rerank5, SF   & 45.30 & 92.23 & 81.50 &  7.51 & 4913 \\
                & 9B & 3-shot, no RAG, SF      & 51.55 & 93.71 & 83.28 & 14.36 & 2286 \\
                & \multicolumn{2}{r}{$\Delta$ (9B $-$ 4B)} & $+6.25$ & $+1.48$ & $+1.78$ & $+6.85$ & $-2627$ \\
\midrule
Mixed           & 4B & 3-shot + rerank5, SF   & 39.46 & 89.60 & 78.86 &  5.45 & 3896 \\
                & 9B & rerank5, SF             & 41.08 & 90.12 & 79.44 &  7.07 & 3016 \\
                & \multicolumn{2}{r}{$\Delta$ (9B $-$ 4B)} & $+1.62$ & $+0.52$ & $+0.58$ & $+1.62$ & $-880$ \\
\bottomrule
\end{tabular}
\caption[Best-configuration quality-cost comparison of Qwen3.5-4B vs.\ Qwen3.5-9B]{Best-configuration quality and cost comparison of Qwen3.5-4B vs.\ Qwen3.5-9B (Spanish dev). $\Delta$ = 9B best $-$ 4B best.}
\label{tab:4b-vs-9b}
\end{table}
